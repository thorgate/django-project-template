...
def test_storybook_generate(cookies, default_project):
    ...
    assert result.project.join('%(repo_name)s/app/.storybook' % default_project).exists()
...
def test_storybook_not_generate(cookies, default_project):
    ...
    assert not result.project.join('%(repo_name)s/app/.storybook' % default_project).exists()
