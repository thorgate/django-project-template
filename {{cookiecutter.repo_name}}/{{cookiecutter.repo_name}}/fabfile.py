...
DJANGO_SITE_URL_TEMPLATE = "DJANGO_SITE_URL=https://${django_site}"
{%- else %}
...
def setup_server(id=None):
    ...
    allowed_hosts = [env.django_site]

    {%- if cookiecutter.frontend_style == 'webapp' %}
    # Create site settings for this env
    site_settings = string.Template(DJANGO_SITE_URL_TEMPLATE).substitute(
        django_site=env.django_site,
    )
    {%- else %}
    allowed_hosts.append(env.node_site)
    ...
    local_env = string.Template(DJANGO_LOCAL_SETTINGS).substitute(
        db_password=db_password,
        secret_key=secret_key,
        site_settings=site_settings,
        allowed_hosts=','.join(set(allowed_hosts)),
        prefix='DJANGO',
    )

    # Upload local settings / env files
    env_file_path = env.code_dir + '/.env'
    put(local_path=StringIO(local_env), remote_path=env_file_path, use_sudo=True)


    # Request additional secrets
    print('Enter Django SENTRY_ENVIRONMENT (staging/production):')
    add_secret_key('DJANGO_SENTRY_ENVIRONMENT', [env_file_path])

    print('Enter Django SENTRY_DSN:')
    add_secret_key('DJANGO_SENTRY_DSN', [env_file_path])

    {%- if cookiecutter.frontend_style == 'spa' %}

    print('Enter Node SENTRY_ENVIRONMENT (staging/production):')
    add_secret_key('RAZZLE_SENTRY_ENVIRONMENT', [env_file_path])

    print('Enter Node SENTRY_DSN:')
    add_secret_key('RAZZLE_SENTRY_DSN', [env_file_path]){% endif %}

    print('Enter Django EMAIL_HOST_PASSWORD:')
    add_secret_key('DJANGO_EMAIL_HOST_PASSWORD', [env_file_path])

    {% if cookiecutter.django_media_engine == 'S3' -%}
    print('Enter Django DJANGO_AWS_ACCESS_KEY_ID:')
    add_secret_key('DJANGO_AWS_ACCESS_KEY_ID', [env_file_path])

    print('Enter Django DJANGO_AWS_SECRET_ACCESS_KEY:')
    add_secret_key('DJANGO_AWS_SECRET_ACCESS_KEY', [env_file_path])
    {%- endif %}{% if cookiecutter.django_media_engine == 'GCS' -%}
    print('Enter Django DJANGO_GS_PROJECT_ID:')
    add_secret_key('DJANGO_GS_PROJECT_ID', [env_file_path])
    with open('./google-credentials-{}.json'.format(env.target)) as f:
        gcs_credentials_json = f.read().replace('\r', '').replace('\n', '')
    add_secret_key('DJANGO_GS_CREDENTIALS', [env_file_path], gcs_credentials_json){% endif %}
