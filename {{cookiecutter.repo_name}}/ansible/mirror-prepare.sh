...
media_dir=`${compose_command} config | yq -r '.services.django.volumes[]' | grep '/files/media' | cut -d: -f1`
...
postgres_dir=`${compose_command} config | yq -r '.services.postgres.volumes[]' | grep '/var/lib/postgresql/data' | cut -d: -f1`
...
postgres_dir=`${compose_command} config | yq -r '.services.postgres.volumes[]' | grep 'db-mirror' | cut -d: -f1`
checkDirectory $postgres_dir
